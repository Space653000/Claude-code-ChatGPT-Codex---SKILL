...
const pluginPaths = [
  ...
  join(repo, "plugins", "mattpocock-skills-complete", ".codex-plugin", "plugin.json"),
];

const { version } = JSON.parse(readFileSync(join(repo, "package.json"), "utf8"));
const manifests = pluginPaths.map((pluginPath) => {
  const source = readFileSync(pluginPath, "utf8");
  return { pluginPath, source, plugin: JSON.parse(source) };
});
const mismatches = manifests.filter(({ plugin }) => plugin.version !== version);

if (mismatches.length === 0) {
  console.log(`Both plugin manifests are ${version} (already in sync)`);
  process.exit(0);
}
